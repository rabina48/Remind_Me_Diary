	...
	grid-template-columns:2fr 3fr 3fr 1fr;
		...
		th{
			...
			<table>
                ...
                error_reporting(0);
                $sql_places="SELECT date,expenditure,income,balance from tbl_journal";
                $result_places=$conn->query($sql_places);
                    if ($result_places->num_rows > 0) {
                        while  ($row =$result_places -> fetch_assoc()){
                        
                            echo "<tr><td>".$row['date']."</td><td>".$row['expenditure']."</td> <td>".$row['income']."</td> <td>".$row['balance']."</td><td></tr>" ;
                        }
                    echo "</table>" ;
                    }
                    else{
